...
function Onboarding({onComplete}){
  ...
  async function finish(){ if(!nombre||!region)return; setSaving(true); const u={nombre,role,region,pais}; await storageSet(KEY_USER,u); onComplete(u); }
  if(step===0)return(
...
function Registrar({user,onSubmit,isFirstTime}){
  ...
  const [saving,setSaving]=useState(false);
  async function handleSubmit(){ if(!precio)return; setSaving(true); await onSubmit({user:user.nombre,role:user.role,region:user.region,pais:user.pais,precio:parseFloat(precio),tipo:user.role==="productor"?"campo":"venta",ts:Date.now(),kg:parseInt(volumen)||0,confiabilidad:80,reportes:1,nota}); setSaving(false); setDone(true); }
...
export default function ExportIA(){
  ...
  useEffect(()=>{
    async function load(){
      try{
        const savedUser=await storageGet(KEY_USER);
        const savedPrices=await storageGet(KEY_PRICES,true);
  ...
  async function handleNewPrice(entry){
    const newEntry={...entry,id:Date.now()};
    const updated=[newEntry,...prices];
    setPrices(updated);
    await storageSet(KEY_PRICES,updated,true);
    setScreen("dashboard");
    setSubScreen(null);
  }

  async function handleLogout(){
    try{await window.storage.delete(KEY_USER);}catch{}
    setUser(null);setScreen("feed");setSubScreen(null);
  }

  if(loading)return<Spinner/>;
  if(!user)return<div style={{background:C.bg,minHeight:"100vh",fontFamily:"'Inter',system-ui,sans-serif"}}><Onboarding onComplete={u=>{setUser(u);setScreen("registrar");}}/></div>;
